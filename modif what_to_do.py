    ...
    for minus in xy:
        for more in xy:
            if not (minus==0 and more==0):
                ...
                if what_in_the_box(search_coordinate,'sheep',map,players,grass):
...
def search_grass (map,players,grass,sheep,distance,owner_grass='e'):
    ...
    for minus in xy:
        for more in xy:
            search_coordinate=[sheep[0]+minus,sheep[1]+more]
            if what_in_the_box(search_coordinate,'grass',map,players,grass):
...
def get_AI_orderz(map_dict,players_dict,grass_dict, player_id):
    ...
    for sheep in players['player_'+str(player_id)]['sheeps']:
        ...
        if (turn <= search_turns or players['player_'+str(player_id)]['nbr_of_grass'] < 30) and len(map['seed'])>0: #for the seed search only turns
            ...
            if free_to_action:
                orders += str(move_sheep(sheep,move_ia(sheep,target_seed,map,players,grass))) #move the sheep to the seed     #verif si move_ia return bien seulement coordonnées
            ...
            if defend_sheep == sheep:      
                orders += str(what_should_do(sheep,0,map,players,grass))
                free_to_action = False
            else :
                orders += str(what_should_do(sheep,1,map,players,grass))
                free_to_action = False
...
def move_ia(location,target,map,players,grass):
    ...
    for minus in xy:
        for more in xy:
            if not (minus==0 and more==0):
                if what_in_the_box([location[0]+minus,location[1]+more],'sheep',map,players,grass) or what_in_the_box([location[0]+minus,location[1]+more],'rock',map,players,grass):
                    wrong.append([location[0]+minus,location[1]+more])
...
def what_should_do(sheep,role,map,players,grass):
    """prend les décision en fonction de la situation du mouton et donne le comportement à adopter
    ...
    if role==0 :
        ...
        ennemy_proche=[]
        herbe=search_grass(map,players,grass,sheep,1000,'a')
        for i in all_ennemy:
            for a in herbe:
                if get_distance(i,a)<5 and i not in ennemy_proche:
                    ennemy_proche.append(i)
