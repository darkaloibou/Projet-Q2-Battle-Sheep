...
players={}
grass={}
def what_in_the_box(xy,search,map,players,grass):
...
def attack(sheep1,sheep2):
    ...
    return orders
def search_sheep (sheep,distance,players):
    ...
    for ennemy_sheep in players[player_team_sheep_2]['sheeps']:
        if get_distance(sheep,ennemy_sheep)<=distance:
            ennemy_sheep_list.append(ennemy_sheep)
    return ennemy_sheep_list
    
def search_grass (map,players,grass,sheep,distance,owner_grass='e'):
    ...
    for player_sheep in players['player_1']['sheeps']:
        if sheep==player_sheep:
            ...
            player_team_sheep_2="player_2"
    if owner_grass=='e':
        for ennemy_grass in grass[player_team_sheep_2]:
            if get_distance(sheep,ennemy_grass)<=distance:
                grass_list.append(ennemy_grass)
    else: 
        for ally_grass in grass[player_team_sheep_1]:
...
def search_seeds (sheep,distance,map):
    ...
    return seed_list
def search_attack(sheep,map,players,grass):
    """return the attack command 
...
def get_AI_orderz(map_dict,players_dict,grass_dict, player_id):
    """Return orders of AI.
    ...
    for sheep in players['player_'+str(player_id)]['sheeps']:
        ...
        if (turn <= search_turns or players['player_'+str(player_id)]['nbr_of_grass'] < 30) and len(map['seed'])>0: #for the seed search only turns
            ...
            if free_to_action:
                orders += str(move_sheep(sheep,move_ia(sheep,target_seed,map_dict,players_dict,grass_dict))) #move the sheep to the seed     #verif si move_ia return bien seulement coordonnées
            ...
            if defend_sheep == sheep:      
                orders += str(what_should_do(sheep,0,map_dict,players_dict,grass_dict,player_id))
                free_to_action = False
                print (orders)
            else :
                orders += str(what_should_do(sheep,1,map_dict,players_dict,grass_dict,player_id))
                free_to_action = False
                print (orders)
...
def what_should_do(sheep,role,map,players,grass,player_id):
        ...
        if mechantbug:
            ...
            for seed in list_seed:
                ...
                if distance<min:
                    min=distance
                    seed_opti=seed
            target=move_ia(sheep,seed_opti,map,players,grass)
