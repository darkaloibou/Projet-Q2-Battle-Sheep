    ...
    while not is_game_over():
        ...
        if type_1 == 'remote':
            ...
            if type_1 == 'human':
                ...
                print (term.white+term.on_black)
                print ('\n'*(plus-3))
                orders1 =  Testing.get_AI_orderz(map,players,grass,1)
                print (orders1)
                time.sleep(1)
                print(term.move_up + term.clear_eos)
            if type_2 == 'remote':
        ...
        if type_2 == 'remote':
            ...
            if type_2 =='human':
                ...
                orders2 =  Testing.get_AI_orderz(map,players,grass,2)
            if type_1 == 'remote':
...
def grass_propagation (mature_grass,life_state): #EMOJI ?
    ...
    for herbs in mature_grass:
            ...
            for x_term in numbers:
                if not what_in_the_box ([(coordinate[0]+x_term),coordinate[1]],'void') and not what_in_the_box([(coordinate[0]+x_term),coordinate[1]],'rock') and not what_in_the_box([(coordinate[0]+x_term),coordinate[1]],'grass') and not what_in_the_box([(coordinate[0]+x_term),coordinate[1]],'spawn') and not what_in_the_box([(coordinate[0]+x_term),coordinate[1]],'seed') :
            ...
            for y_term in numbers:
                if not what_in_the_box ([(coordinate[0]),(coordinate[1]+y_term)],'void') and not what_in_the_box([(coordinate[0]),(coordinate[1]+y_term)],'rock') and not what_in_the_box([(coordinate[0]),(coordinate[1]+y_term)],'grass') and not what_in_the_box([(coordinate[0]),(coordinate[1]+y_term)],'spawn') and not what_in_the_box([(coordinate[0]),(coordinate[1]+y_term)],'seed'):
...
def game_function(player_1_orders,player_2_orders):   #j'ai changé la spécification mais il faut que je demande au tuteur ou assistant pour la mettre dans se dossier
    ...
    another_list_1=[]
    another_list_2=[]
    for kill_sheep in players['player_1']['sheeps']:
        if players['player_1']['sheeps'][kill_sheep][0]<=0:
            another_list_1.append(kill_sheep)
    for kill_other_sheep in players['player_2']['sheeps']:
        if players['player_2']['sheeps'][kill_other_sheep][0]<=0:
            another_list_2.append(kill_other_sheep)
    for another_sheep in another_list_1:
        del players['player_1']['sheeps'][another_sheep]
    for another_sheep_2 in another_list_2:
        del players['player_2']['sheeps'][another_sheep_2]
    for order in player_1_orders:
        if '@' in order:
            order_string=order.split(":")
...
def what_in_the_box(xy,search):
    ...
    if search=='seed':
        for seed in map['seed']:
            seed_coordinate=(map['seed'][seed][0],map['seed'][seed][1])
            if xy==seed_coordinate:
                return True
        return False
